...
class CustomHelpCommand(commands.Cog):
    ...
    async def help_cat(self, ctx):
        embed = discord.Embed(title="Help by categories", description="Voici les catégories d'aide disponibles :", color=discord.Color.blue())
        for cog_name, cog in self.bot.cogs.items():
            command_list = "\n".join([f"{self.bot.command_prefix}{command.name}: {command.short_doc or 'No description'}" for command in cog.get_commands() if not command.hidden])
            if command_list:
    ...
    async def send_bot_help(self, mapping):
        embed = discord.Embed(title="Commands of the bot", color=discord.Color.blue())
        for cog, commands in mapping.items():
            ...
            command_list = "\n".join([f"{self.bot.command_prefix}{command.name}: {command.help or 'No description'}" for command in commands if not command.hidden])
            if command_list:
                embed.add_field(name=cog_name, value=command_list, inline=False)
        destination = self.get_destination()
        for page in self.paginator.pages:
            embed = discord.Embed(title="Bot help", description=page, color=discord.Color.blue())
    ...
    async def send_command_help(self, command):
        ...
        embed.add_field(name="Use", value=self.get_command_signature(command), inline=False)
        destination = self.get_destination()
        for page in self.paginator.pages:
            embed = discord.Embed(title="Bot help", description=page, color=discord.Color.blue())
            await destination.send(embed=embed)
    ...
    async def send_cog_help(self, cog):
        ...
        command_list = "\n".join([f"{self.bot.command_prefix}{command.name}: {command.help or 'No description'}" for command in cog.get_commands() if not command.hidden])
        if command_list:
